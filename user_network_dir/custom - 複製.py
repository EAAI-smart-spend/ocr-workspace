    ...
    def forward(self, batch_I):
        ...
        if torch.__version__ > "1.2.0":
            batch_I_r = F.grid_sample(
                batch_I, build_P_prime_reshape, padding_mode='zeros', align_corners=True)
        else:
            batch_I_r = F.grid_sample(
                batch_I, build_P_prime_reshape, padding_mode='zeros')
...
class GridGenerator(nn.Module):
    ...
    def _build_P(self, I_r_width, I_r_height):
        ...
        return P.reshape([-1, 2])  # n (= I_r_width x I_r_height) x 2

    def _build_P_hat(self, F, C, P):
        n = P.shape[0]  # n (= I_r_width x I_r_height)
    ...
    def build_P_prime(self, batch_C_prime):
        ...
        batch_C_prime_with_zeros = torch.cat((batch_C_prime, torch.zeros(
            ...
            batch_size, 3, 2).float().to(batch_C_prime.device)), dim=1)
        # batch_size x F+3 x 2
